...
public:
    ...
    // translate it into the VEX intermediate language. This is fixed with newer versions like 3.16.1

    explicit RandomInt(int low, int high) : dist{low, high} { 
...
public:
    explicit RandomDouble(double low, double high) : dist{low, high} {
        re.seed(std::chrono::system_clock::now().time_since_epoch().count());         
    }
